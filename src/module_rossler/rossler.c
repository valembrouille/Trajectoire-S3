...
struct point ** rossler (struct point * M, struct param * Param){
    float dt = 0.01;
    int N = M->T_max / dt;
    ...
    for (int i=1; i<N; i++){
        ...
        L[i]->y = L[i-1]->y + (L[i-1]->x + Param->a * L[i-1]->y) * dt;
        L[i]->z = L[i-1]->z + (Param->b + L[i-1]->z * (L[i-1]->x - Param->c)) * dt;
